...
constexpr int defaultSafeOffset = 9;

Board::Board(int rows, int cols, int mines) : rows(rows), cols(cols), mines(mines) {
	int maxMines = rows * cols - defaultSafeOffset;
	mines = std::max(1, std::min(mines, maxMines));
...
void Board::placeMines(int safeRow, int safeCol) {
	std::vector<std::pair<int, int>> candidates = getCandidates(safeRow, safeCol);
	std::vector<std::pair<int, int>> chosenMines;
	std::ranges::sample(candidates, std::back_inserter(chosenMines), mines, gen);
	for (const auto& [r, c] : chosenMines) {
		...
		for (const auto& dir : directions) {
			...
			if (isValidMove(nr, nc)) {
				...
				grid[neighborIndex].neighborMines++;
			}
		}
	}
}
...
std::vector<std::pair<int, int>> Board::getCandidates(int safeRow, int safeCol) const {
	...
	candidates.reserve(rows * cols - defaultSafeOffset);
...
int Board::countNeighborFlags(int row, int col) const {
	...
	for (const auto& dir : directions) {
		...
		int index = getIndex(r, c);
		if (isValidMove(r, c) && grid[index].isFlagged) {
			count++;
		}
	}
	return count;
...
void Board::Chord(int row, int col) {
	if (!canBeChorded(row, col)) return;
	for (const auto& dir : directions) {
		int r = row + dir.first;
		int c = col + dir.second;
		if (isValidMove(r, c)) {
